...
const initialForm = { customer_name: "", phone: "", email: "", province: "Maputo Cidade", city: "", address_reference: "", preferred_delivery_time: "" };
...
export function CheckoutDialog({ open, onOpenChange, product, variant }: { open: boolean; onOpenChange: (open: boolean) => void; product: Product; variant: string }) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setServerError("");
    const payload = { ...form, product_name: product.name, product_variant: variant, subtotal: product.price * quantity };
  ...
  return <Dialog open={open} onOpenChange={close}><DialogContent className="bottom-0 left-0 top-auto max-h-[94svh] w-full max-w-none translate-x-0 translate-y-0 overflow-y-auto rounded-t-2xl border-b-0 p-0 sm:bottom-auto sm:left-1/2 sm:top-1/2 sm:max-w-2xl sm:-translate-x-1/2 sm:-translate-y-1/2 sm:rounded-lg sm:border-b">
    {success ? <div className="grid min-h-[420px] place-items-center p-8 text-center"><div><span className="mx-auto grid h-16 w-16 place-items-center rounded-full bg-success text-success-foreground"><Check className="h-7 w-7"/></span><h2 className="mt-6 font-display text-4xl">Pedido realizado com sucesso!</h2><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-muted-foreground">Entraremos em contacto em breve para confirmar a entrega.</p><Button className="mt-7" onClick={() => close(false)}>Continuar a explorar</Button></div></div> : <>
      ...
      <form onSubmit={handleSubmit} className="p-5 sm:p-7">
        <div className="grid grid-cols-[72px_minmax(0,1fr)] gap-4 rounded-lg bg-soft p-3"><img src={product.image} alt="" className="aspect-square w-[72px] rounded-md object-cover"/><div className="min-w-0"><p className="truncate font-display text-xl">{product.name}</p><p className="text-xs text-muted-foreground">{variant}</p><div className="mt-2 flex items-center gap-2"><span className="text-xs font-semibold">Qtd.</span><button type="button" className="h-8 w-8 rounded border" onClick={()=>setQuantity(q=>Math.max(1,q-1))}>−</button><span className="w-6 text-center text-sm">{quantity}</span><button type="button" className="h-8 w-8 rounded border" onClick={()=>setQuantity(q=>q+1)}>+</button></div><div className="mt-2 grid grid-cols-[1fr_auto] gap-y-1 text-xs"><span>Subtotal</span><strong>{formatPrice(product.price * quantity)}</strong><span className="flex items-center gap-1">Custo de frete <span title="Frete grátis para Maputo/Matola. Para outras províncias, a taxa é calculada após confirmação da localização."><CircleHelp className="h-3 w-3"/></span></span><strong>Grátis*</strong><span className="mt-2 border-t border-border pt-2 font-semibold">Valor total a pagar</span><strong className="mt-2 border-t border-border pt-2">{formatPrice(product.price * quantity)}</strong></div></div></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2">
          ...
          <Field label="Hora Preferencial *" error={errors.preferred_delivery_time}><div className="flex gap-2"><button type="button" className={`flex-1 rounded-md border px-2 py-3 text-xs ${form.preferred_delivery_time==="Manhã (08h - 12h)"?"bg-foreground text-background":"bg-background"}`} onClick={()=>update("preferred_delivery_time","Manhã (08h - 12h)")}>Manhã</button><button type="button" className={`flex-1 rounded-md border px-2 py-3 text-xs ${form.preferred_delivery_time==="Tarde (13h - 17h)"?"bg-foreground text-background":"bg-background"}`} onClick={()=>update("preferred_delivery_time","Tarde (13h - 17h)")}>Tarde</button></div></Field>
        ...
        <Button type="submit" size="lg" className="mt-6 h-12 w-full" disabled={pending}>{pending ? <><LoaderCircle className="animate-spin"/>A concluir...</> : `Concluir o pedido — ${formatPrice(product.price * quantity)}`}</Button>
